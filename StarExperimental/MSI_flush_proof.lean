...
/-- Lo spec sequenziale: una memoria e, per ogni cache, la coda esterna. Gli eventi portano l'indice
della cache (`MSIExternalEvent n`), come `msi_step_external`: così lo spec agisce sulla stessa coda
dell'implementazione e il suo passo è determinato dall'evento. Le richieste vengono servite in
ordine FIFO; `ld_rs v` richiede `v` uguale alla memoria, `st_rs` scrive il valore in testa. -/
...
/-- La parte "di quiete" della relazione di flush: `i` è vuoto e coincide con `s` sulla memoria.
Nessun messaggio in volo (le code `queue_cp`/`queue_pc` di ogni cache e le loro copie
`queue_cip`/`queue_pci` lato parent sono vuote), tutte le cache in `I`, la directory del parent
tutta a `I` (serve per il ritorno a flush: senza righe a `I` il parent non può concedere `M`) e il
valore del parent uguale alla memoria dello spec. Le `extqueue` sono trattate a parte, in `flush`. -/
...
/-- La relazione di flush completa: `flush0` più l'uguaglianza delle `extqueue` con quelle dello spec.
È possibile perché gli eventi esterni portano l'indice della cache (`MSIExternalEvent n`) e
`seq_step` è etichettato dagli stessi eventi: spec e implementazione agiscono sulla stessa coda. -/
def flush (i : MSIState n) (s : SeqState n) : Prop :=
  flush0 i s ∧ ∀ k, (i.caches k).extqueue = s.extqueue k


/-! ## Le relazioni del framework (`Star/Commute/ARS.lean`) istanziate su MSI

`flush0` è la relazione di flush0, `msi_step` e `seq_step` sono i `Method` di implementazione e spec
(entrambi etichettati da `Event`), `msi_rule` è la `Rule` interna. Gli enunciati che seguono sono
esattamente `relation_method`, `relation_flush_method`, `relation_init` e `relation_flush` di ARS. -/
...
def msi_rule (n : Nat) : Rule (MSIState n) := fun a b => ∃ e, msi_step_internal a e b

theorem msi_rule_eq_atrans : msi_rule n = (@MSI n).atrans := rfl
...
theorem msi_relation_init0 (i : MSIState n) (hi : msi_init i) :
...
flushed per `s`: code sincronizzate (`MSIView.synced`), *token* (se la riga `k` della directory è
...
def tokenM (x : MSIState n) (k : Fin n) : Prop :=
...
def tokenS (x : MSIState n) (k : Fin n) : Prop :=
...
structure flushInv (x : MSIState n) (s : SeqState n) : Prop where
  synced : MSIView.synced x
  rowM : ∀ k, x.parent.shared_state k = Bstate.M → tokenM x k
...
def μ2 (x : MSIState n) : Nat := Finset.univ.sum (fun k => (x.caches k).queue_cp.countP isRequest)
def μ3 (x : MSIState n) : Nat := Finset.univ.sum (fun k => (x.caches k).queue_pc.countP isInval)
...
theorem inv_of_flush {x : MSIState n} {s : SeqState n} (hf : flush0 x s) : flushInv x s := by
  obtain ⟨hc, hp, hv⟩ := hf
...
/-- Il valore di una cache non in `I` resta quello della memoria dello spec. -/
theorem inv_step_cache_aux3 {s1 c' : CacheState} {e : CacheInternalEvent} {m : Value}
    (h : cache_msi_step_internal s1 e c')
    (hcv : s1.state ≠ Bstate.I → s1.value = m)
    (hgv : ∀ v, (PCEvent.rsM v ∈ s1.queue_pc ∨ PCEvent.rsS v ∈ s1.queue_pc) → v = m) :
  ...
  | upgrade_from_I_rsS v j hj hI => intro _; exact hgv v (Or.inr (List.mem_of_getElem? hj))
...
theorem inv_step_cache_aux4 {s1 c' : CacheState} {e : CacheInternalEvent}
    (h : cache_msi_step_internal s1 e c') (a : PCEvent) (ha : a ∈ c'.queue_pc) :
    a ∈ s1.queue_pc := by
  cases h with
...
theorem inv_step_cache {x : MSIState n} {s : SeqState n} {k : Fin n} {e : CacheInternalEvent}
    {c' : CacheState} (hI : flushInv x s) (h : cache_msi_step_internal (x.caches k) e c') :
  ...
  have hsync' := MSIView.synced_step hsync (msi_step_internal.cache x c' k e h)
  ...
  · intro k' hk'
    ...
    · subst hk
      ...
      exact inv_step_cache_aux1 h (hM _ hk')
  ...
  · intro k' hne
    ...
    · subst hk
      ...
      exact inv_step_cache_aux3 h (hcv _) (hgv _) hne
  ...
  · intro k' v hv
    ...
    · subst hk
      ...
      exact hgv _ v (hv.imp (inv_step_cache_aux4 h _) (inv_step_cache_aux4 h _))
...
/-- Ausiliario di `inv_step_parent`: l'invariante nello stato dopo un passo del parent segue dalle
condizioni all'indice `k` (le altre cache e le altre righe non cambiano). -/
theorem inv_step_parent_aux1 {x : MSIState n} {s : SeqState n} {k : Fin n} {p' : ParentState n}
    (hI : flushInv x s)
    (hsync : MSIView.synced
    ...
    (hM : p'.shared_state k = Bstate.M →
      (x.caches k).state = Bstate.M ∨ (∃ v, PCEvent.rsM v ∈ p'.queue_pci k)
    ...
    (hgv : ∀ v, (PCEvent.rsM v ∈ p'.queue_pci k ∨ PCEvent.rsS v ∈ p'.queue_pci k) → v = s.memory)
...
theorem inv_step_parent {x : MSIState n} {s : SeqState n} {k : Fin n}
    {e : ParentUpdQueueInternalEvent n} {p' : ParentState n}
    (hI : flushInv x s) (h : parent_msi_step x.parent (.upd_queue e k) p') :
    flushInv { x with caches := update_Fin k { x.caches k with queue_cp := p'.queue_cip k,
                                                                queue_pc := p'.queue_pci k } x.caches,
                      parent := p' } s := by
  have hsync' := MSIView.synced_step hI.synced (msi_step_internal.parent_upd_queue x p' e k h)
  have hother : ∀ k', ¬ k' = k → p'.shared_state k' = x.parent.shared_state k' :=
    fun k' hk => (MSIView.parent_step_local h k' hk).2.2
  ...
  case downgrade_from_M_rq1 v j hj =>
    refine inv_step_parent_aux1 hI hsync' ?_ hother ?_ ?_ ?_ ?_
  ...
  case downgrade_from_M_rq2 =>
    refine inv_step_parent_aux1 hI hsync' hval hother ?_ ?_ ?_ ?_
    · intro hc; simp at hc
    · intro hc; simp at hc
    · intro v' hmem
      apply hgv k v'
  ...
  case upgrade_to_M_data_avilable_rq1 =>
    ...
    · intro v' hmem
      simp only [update_Fin_gss] at hmem
      rcases inv_step_parent_aux2 hmem with h | h | h
  ...
  all_goals
    ...
    · intro hc
      ...
      unfold tokenM at ht
      simp only [update_Fin_gss]
      rcases ht with h | ⟨v, h⟩ | ⟨v, h⟩
      · exact Or.inl h
      · rw [← hpc] at h
        exact Or.inr (Or.inl ⟨v, List.mem_append_left _ h⟩)
      · rw [← hcp] at h
        exact Or.inr (Or.inr ⟨v, h⟩)
    · intro hc
      have ht := hS k hc
      unfold tokenS at ht
      simp only [update_Fin_gss]
...
theorem inv_step {x y : MSIState n} {s : SeqState n} {e : MSIInternalEvent n}
    (hI : flushInv x s) (h : msi_step_internal x e y) : flushInv y s := by
  cases h with
  | cache c' k e' hc => exact inv_step_cache hI hc
  | parent_upd_queue p' e' k hp => exact inv_step_parent hI hp
  | parent_no_queue p' e' k hp => cases hp

theorem inv_trans {x y : MSIState n} {s : SeqState n}
    (hI : flushInv x s) (h : trans_refl (msi_rule n) x y) : flushInv y s := by
  revert hI
  induction h with
  | refl => exact id
  | step hab _ ih =>
...
theorem phase1_step_release {x : MSIState n} {s : SeqState n} {k : Fin n} (_hI : flushInv x s)
    (hk : (x.caches k).state ≠ Bstate.I) : ∃ y, msi_rule n x y ∧ μ1 y < μ1 x := by
  cases hst : (x.caches k).state with
  | I => exact absurd hst hk
  | M =>
    refine ⟨_, msi_rule_of_step (msi_step_internal.cache _ _ k _
...
theorem phase1_step_proc {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat} {m : CPEvent}
  ...
  cases m with
  | rsIμ v =>
    refine ⟨_, msi_rule_of_step (msi_step_internal.parent_upd_queue x _ _ k
      (parent_msi_step.downgrade_from_M_rq1 x.parent v k j hj')), ?_⟩
    apply phase1_step_proc_aux1
    simp only [m1, update_Fin_gss, hs1, hs2]
    omega
  | rsIσ =>
    refine ⟨_, msi_rule_of_step (msi_step_internal.parent_upd_queue x _ _ k
      (parent_msi_step.downgrade_from_M_rq2 x.parent k j hj')), ?_⟩
    apply phase1_step_proc_aux1
    simp only [m1, update_Fin_gss, hs1, hs2]
    omega
...
theorem phase1_step_take {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat} {m : PCEvent}
  ...
  | rsM v =>
    have hstep := cache_msi_step_internal.upgrade_from_I_rs (x.caches k) v j hj (hall k)
    refine ⟨_, msi_rule_of_step (msi_step_internal.cache x _ k _ hstep), phase1_step_take_aux1 ?_⟩
    ...
    omega
  | rsS v =>
    have hstep := cache_msi_step_internal.upgrade_from_I_rsS (x.caches k) v j hj (hall k)
    refine ⟨_, msi_rule_of_step (msi_step_internal.cache x _ k _ hstep), phase1_step_take_aux1 ?_⟩
    have hc := countP_eraseIdx_pos (p := isGrant) hj rfl
    unfold m1
    simp only [update_Fin_gss]
...
theorem phase1_step {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (hpos : 0 < μ1 x) :
    ∃ y, msi_rule n x y ∧ μ1 y < μ1 x := by
...
theorem phase1 {x : MSIState n} {s : SeqState n} (hI : flushInv x s) :
    ∃ y, trans_refl (msi_rule n) x y ∧ flushInv y s ∧ μ1 y = 0 := by
  suffices H : ∀ m, ∀ x : MSIState n, flushInv x s → μ1 x ≤ m →
      ∃ y, trans_refl (msi_rule n) x y ∧ flushInv y s ∧ μ1 y = 0 from H _ x hI le_rfl
  intro m
  induction m with
  | zero =>
    intro x hI hm
  ...
  | succ m ih =>
    ...
    · obtain ⟨y, hxy, hlt⟩ := phase1_step hI (Nat.pos_of_ne_zero h0)
      ...
      exact ⟨z, trans_refl.step (msi_rule_of_step he) hyz, hIz, hz⟩
...
theorem quiet_rows {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (h0 : μ1 x = 0) (k : Fin n) :
...
theorem phase2_step_M {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat} (hI : flushInv x s)
    (h0 : μ1 x = 0) (hj : (x.caches k).queue_cp[j]? = some CPEvent.rqM) :
    ∃ y, trans_refl (msi_rule n) x y ∧ μ1 y = 0 ∧ μ2 y < μ2 x := by
  -- Fatti da `h0` e dall'invariante.
  have hs : MSIView.synced x := hI.synced
  ...
  obtain ⟨x1, step1, h1s, h1state, h1pc, h1cp, h1oth⟩ :
    ...
    have hstep := msi_step_internal.parent_upd_queue x _ _ k
      (parent_msi_step.upgrade_to_M_data_avilable_rq1 x.parent k j hj1 hrows)
    refine ⟨_, msi_rule_of_step hstep, MSIView.synced_step hs hstep, ?_, ?_, ?_, ?_⟩
  ...
  obtain ⟨x2, step2, h2s, h2state, h2pc, h2cp, h2val, h2oth⟩ :
    ...
    have hstep := msi_step_internal.cache x1 _ k _
      (cache_msi_step_internal.upgrade_from_I_rs (x1.caches k) x.parent.value
        (x.caches k).queue_pc.length hg3 h1state)
    refine ⟨_, msi_rule_of_step hstep, MSIView.synced_step h1s hstep, ?_, ?_, ?_, ?_, ?_⟩
    · simp only [update_Fin_gss]
  ...
  obtain ⟨x3, step3, h3s, h3state, h3pc, h3cp, h3oth⟩ :
    ...
    refine ⟨_, msi_rule_of_step hstep, MSIView.synced_step h2s hstep, ?_, ?_, ?_, ?_⟩
    · simp only [update_Fin_gss]
  ...
  obtain ⟨x4, step4, h4state, h4pc, h4cp, h4oth⟩ :
    ...
    have hstep := msi_step_internal.parent_upd_queue x3 _ _ k
      (parent_msi_step.downgrade_from_M_rq1 x3.parent x.parent.value k _ hg6)
    refine ⟨_, msi_rule_of_step hstep, ?_, ?_, ?_, ?_⟩
...
theorem phase2_step_S {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat} (hI : flushInv x s)
  ...
  refine ⟨_,
    trans_refl.step (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
        (parent_msi_step.upgrade_to_M_data_avilable_rq2 _ k j hj' g2 g3)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.cache _ _ k _
        (cache_msi_step_internal.upgrade_from_I_rsS _ x.parent.value (x.parent.queue_pci k).length
          ?g4 ?g5)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.cache _ _ k _
        (cache_msi_step_internal.rq_data_not_available1 _ ?g6)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
        (parent_msi_step.downgrade_from_M_rq2 _ k ((x.parent.queue_cip k).eraseIdx j).length ?g7)))
...
theorem phase2_step {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (h0 : μ1 x = 0)
    (hpos : 0 < μ2 x) : ∃ y, trans_refl (msi_rule n) x y ∧ μ1 y = 0 ∧ μ2 y < μ2 x := by
  unfold μ2 at hpos
  obtain ⟨k, hk⟩ := exists_pos_of_sum_pos hpos
...
/-- Transitività di `trans_refl` (helper per `phase2`, `phase3`, `msi_relation_flush`). -/
...
theorem phase2 {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (h0 : μ1 x = 0) :
    ∃ y, trans_refl (msi_rule n) x y ∧ flushInv y s ∧ μ1 y = 0 ∧ μ2 y = 0 := by
  suffices H : ∀ m, ∀ x : MSIState n, flushInv x s → μ1 x = 0 → μ2 x ≤ m →
      ∃ y, trans_refl (msi_rule n) x y ∧ flushInv y s ∧ μ1 y = 0 ∧ μ2 y = 0 from
    H _ x hI h0 le_rfl
  intro m
  induction m with
  | zero =>
...
theorem phase3_step_M {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat} (hI : flushInv x s)
  ...
  refine ⟨_, trans_refl.step (msi_rule_of_step (msi_step_internal.cache x _ k _
      (cache_msi_step_internal.upgrade_from_I_rq (x.caches k) hc0)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
      (parent_msi_step.upgrade_to_M_data_avilable_rq1 _ k (x.caches k).queue_cp.length ?g1 ?g2)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.cache _ _ k _
      (cache_msi_step_internal.upgrade_from_I_rs _ x.parent.value (x.caches k).queue_pc.length ?g3 ?g4)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.cache _ _ k _
      (cache_msi_step_internal.downgrade_from_M_rs _ j ?g5 ?g6)))
    (trans_refl.step (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
      (parent_msi_step.downgrade_from_M_rq1 _ x.parent.value k (x.caches k).queue_cp.length ?g7)))
...
theorem phase3_step_S_aux1 {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat}
  ...
  case chain =>
    ...
    refine trans_refl.step (msi_rule_of_step (msi_step_internal.cache x _ k _
      (cache_msi_step_internal.upgrade_from_I_rq1 _ hI0))) ?_
    -- 2. il parent concede `S`
    refine trans_refl.step (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
      (parent_msi_step.upgrade_to_M_data_avilable_rq2 _ k (x.caches k).queue_cp.length
        ?g21 ?g22 ?g23))) ?_
    ...
    refine trans_refl.step (msi_rule_of_step (msi_step_internal.cache _ _ k _
      (cache_msi_step_internal.upgrade_from_I_rsS _ x.parent.value (x.caches k).queue_pc.length
        ?g31 ?g32))) ?_
    ...
    refine trans_refl.step (msi_rule_of_step (msi_step_internal.cache _ _ k _
      (cache_msi_step_internal.downgrade_from_M_rs1 _ j ?g41 ?g42))) ?_
    case g41 => simp only [update_Fin_gss, eraseIdx_concat, hj]
    case g42 => simp only [update_Fin_gss]
    -- 5. il parent prende il rilascio `rsIσ`
    refine trans_refl.step (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
      (parent_msi_step.downgrade_from_M_rq2 _ k (x.caches k).queue_cp.length ?g51))) ?_
...
theorem phase3_step_S {x : MSIState n} {s : SeqState n} {k : Fin n} {j : Nat} (hI : flushInv x s)
...
theorem phase3_step {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (h0 : μ1 x = 0)
...
theorem phase3 {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (h0 : μ1 x = 0) (h2 : μ2 x = 0) :
    ∃ y, trans_refl (msi_rule n) x y ∧ flushInv y s ∧ μ1 y = 0 ∧ μ2 y = 0 ∧ μ3 y = 0 := by
  suffices H : ∀ m, ∀ x : MSIState n, flushInv x s → μ1 x = 0 → μ2 x = 0 → μ3 x ≤ m →
      ∃ y, trans_refl (msi_rule n) x y ∧ flushInv y s ∧ μ1 y = 0 ∧ μ2 y = 0 ∧ μ3 y = 0 from
...
theorem flush_of_quiet {x : MSIState n} {s : SeqState n} (hI : flushInv x s) (h0 : μ1 x = 0)
...
theorem msi_relation_flush0 (i i' : MSIState n) (s : SeqState n) :
    relation_flush flush0 i i' s (msi_rule n) := by
...
/-- Il caso a un passo del ritorno a flush (per `flush0`), da `msi_relation_flush0`. -/
theorem msi_relation_flush_one (i i' : MSIState n) (s : SeqState n) (e : MSIInternalEvent n) :
    flush0 i s → msi_step_internal i e i' →
    ∃ i'', trans_refl (msi_rule n) i' i'' ∧ flush0 i'' s := by
  intro hf h
  exact msi_relation_flush0 i i' s hf (trans_refl.step (msi_rule_of_step h) trans_refl.refl)
...
theorem msi_flush_step_φ_ind (i i' : MSIState n) (s : SeqState n) (e : MSIInternalEvent n)
    (hf : flush0 i s) (h : msi_step_internal i e i') : φ_ind flush0 (msi_rule n) i' s := by
  obtain ⟨i'', htr, hf''⟩ := msi_relation_flush_one i i' s e hf h
...
`has_diamond_property_on Reach (trans_refl (msi_rule n))`: in ogni stato `a` raggiungibile da
...
def mtokC (c : CacheState) : Nat :=
  (if c.state = Bstate.M then 1 else 0) + c.queue_pc.countP MSIView.isGrantM
    + c.queue_cp.countP MSIView.isReleaseM

/-- Token `S` di una cache: stato `S`, grant `rsS` in volo, rilasci `rsIσ` in volo. -/
def stokC (c : CacheState) : Nat :=
  (if c.state = Bstate.S then 1 else 0) + c.queue_pc.countP MSIView.isGrantS
    + c.queue_cp.countP MSIView.isReleaseS
...
structure cohInv (x : MSIState n) : Prop where
  synced : MSIView.synced x
...
structure LV (x : MSIState n) (m : Value) : Prop where
...
theorem tok_cache_step {c c' : CacheState} {e : CacheInternalEvent}
    (h : cache_msi_step_internal c e c') : mtokC c' = mtokC c ∧ stokC c' = stokC c := by
  cases h with
  | rq_data_not_available hM =>
    unfold mtokC stokC
    simp [hM, List.countP_append, MSIView.isReleaseM, MSIView.isReleaseS]
    omega
  | rq_data_not_available1 hS =>
    unfold mtokC stokC
    simp [hS, List.countP_append, MSIView.isReleaseM, MSIView.isReleaseS]
    omega
  | upgrade_from_I_rq hI =>
    unfold mtokC stokC
    simp [hI, List.countP_append, MSIView.isReleaseM, MSIView.isReleaseS]
  | upgrade_from_I_rq1 hI =>
    unfold mtokC stokC
    simp [hI, List.countP_append, MSIView.isReleaseM, MSIView.isReleaseS]
  | upgrade_from_I_rs v j hj hI =>
    have h1 := countP_eraseIdx_pos hj (rfl : MSIView.isGrantM (PCEvent.rsM v) = true)
    have h2 := countP_eraseIdx_neg hj (rfl : MSIView.isGrantS (PCEvent.rsM v) = false)
    unfold mtokC stokC
    simp [hI]
    omega
  | upgrade_from_I_rsS v j hj hI =>
    have h1 := countP_eraseIdx_pos hj (rfl : MSIView.isGrantS (PCEvent.rsS v) = true)
    have h2 := countP_eraseIdx_neg hj (rfl : MSIView.isGrantM (PCEvent.rsS v) = false)
    unfold mtokC stokC
    simp [hI]
    omega
  | downgrade_from_M_rs j hj hM =>
    have h1 := countP_eraseIdx_neg hj (rfl : MSIView.isGrantM PCEvent.rqIμ = false)
    have h2 := countP_eraseIdx_neg hj (rfl : MSIView.isGrantS PCEvent.rqIμ = false)
    unfold mtokC stokC
    simp [hM, List.countP_append, MSIView.isReleaseM, MSIView.isReleaseS]
    omega
  | downgrade_from_M_rs1 j hj hS =>
    have h1 := countP_eraseIdx_neg hj (rfl : MSIView.isGrantM PCEvent.rqIσ = false)
    have h2 := countP_eraseIdx_neg hj (rfl : MSIView.isGrantS PCEvent.rqIσ = false)
    unfold mtokC stokC
    simp [hS, List.countP_append, MSIView.isReleaseM, MSIView.isReleaseS]
    omega
...
theorem tok_cache_step_ext {c c' : CacheState} {e : Event}
    (h : cache_msi_step c e c') : mtokC c' = mtokC c ∧ stokC c' = stokC c := by
  cases h with
  | ld_rq => exact ⟨rfl, rfl⟩
...
theorem mtok_one_cases {x : MSIState n} {k : Fin n} (h : mtok x k = 1) :
    ((x.caches k).state = Bstate.M ∧ (x.caches k).queue_pc.countP MSIView.isGrantM = 0
        ∧ (x.caches k).queue_cp.countP MSIView.isReleaseM = 0)
    ∨ ((x.caches k).state ≠ Bstate.M ∧ (∃ v, PCEvent.rsM v ∈ (x.caches k).queue_pc)
        ∧ (x.caches k).queue_cp.countP MSIView.isReleaseM = 0)
    ∨ ((x.caches k).state ≠ Bstate.M ∧ (x.caches k).queue_pc.countP MSIView.isGrantM = 0
  ...
  · rw [if_neg hM] at h
    by_cases hg : (x.caches k).queue_pc.countP MSIView.isGrantM = 0
    · have hpos : 0 < (x.caches k).queue_cp.countP MSIView.isReleaseM := by omega
      obtain ⟨m, hm, hp⟩ := List.countP_pos_iff.mp hpos
      refine Or.inr (Or.inr ⟨hM, hg, ?_⟩)
      cases m with
      | rsIμ v => exact ⟨v, hm⟩
      | rsIσ => simp [MSIView.isReleaseM] at hp
      | rqS => simp [MSIView.isReleaseM] at hp
      | rqM => simp [MSIView.isReleaseM] at hp
    · have hpos : 0 < (x.caches k).queue_pc.countP MSIView.isGrantM := Nat.pos_of_ne_zero hg
      obtain ⟨m, hm, hp⟩ := List.countP_pos_iff.mp hpos
      refine Or.inr (Or.inl ⟨hM, ?_, by omega⟩)
      cases m with
      | rsM v => exact ⟨v, hm⟩
      | rsS v => simp [MSIView.isGrantM] at hp
      | rqIμ => simp [MSIView.isGrantM] at hp
      | rqIσ => simp [MSIView.isGrantM] at hp

theorem tokenS_of_stok_pos {x : MSIState n} {k : Fin n} (h : 0 < stok x k) : tokenS x k := by
  unfold stok stokC at h
  unfold tokenS
  by_cases hS : (x.caches k).state = Bstate.S
  · exact Or.inl hS
  · rw [if_neg hS] at h
    by_cases hg : (x.caches k).queue_pc.countP MSIView.isGrantS = 0
    · have hpos : 0 < (x.caches k).queue_cp.countP MSIView.isReleaseS := by omega
      obtain ⟨m, hm, hp⟩ := List.countP_pos_iff.mp hpos
      cases m with
      | rsIσ => exact Or.inr (Or.inr hm)
      | rsIμ v => simp [MSIView.isReleaseS] at hp
      | rqS => simp [MSIView.isReleaseS] at hp
      | rqM => simp [MSIView.isReleaseS] at hp
    · have hpos : 0 < (x.caches k).queue_pc.countP MSIView.isGrantS := Nat.pos_of_ne_zero hg
      obtain ⟨m, hm, hp⟩ := List.countP_pos_iff.mp hpos
      cases m with
      | rsS v => exact Or.inr (Or.inl ⟨v, hm⟩)
      | rsM v => simp [MSIView.isGrantS] at hp
      | rqIμ => simp [MSIView.isGrantS] at hp
      | rqIσ => simp [MSIView.isGrantS] at hp
...
theorem mtok_zero_facts {x : MSIState n} {k : Fin n} (h : mtok x k = 0) :
  ...
  · intro hM
    ...
    omega
  · intro v hv
    have hg : (x.caches k).queue_pc.countP MSIView.isGrantM = 0 := by omega
    exact List.countP_eq_zero.mp hg _ hv rfl
  · intro v hv
    have hr : (x.caches k).queue_cp.countP MSIView.isReleaseM = 0 := by omega
...
theorem stok_zero_facts {x : MSIState n} {k : Fin n} (h : stok x k = 0) :
  ...
  · intro hS
    ...
    omega
  · intro v hv
    have hg : (x.caches k).queue_pc.countP MSIView.isGrantS = 0 := by omega
    exact List.countP_eq_zero.mp hg _ hv rfl
  · intro hv
    have hr : (x.caches k).queue_cp.countP MSIView.isReleaseS = 0 := by omega
    exact List.countP_eq_zero.mp hr _ hv rfl

/-! ### `cohInv`: vale in `default`, conservato da passi interni ed esterni -/

theorem coh_default : cohInv (default : MSIState n) := by
...
theorem coh_step_cache_aux1 {x : MSIState n} {k : Fin n} {c' : CacheState}
...
theorem coh_step_cache_aux2 {x : MSIState n} {k : Fin n} {c' : CacheState}
...
theorem coh_step_cache {x : MSIState n} {k : Fin n} {e : CacheInternalEvent} {c' : CacheState}
    (hc : cohInv x) (h : cache_msi_step_internal (x.caches k) e c') :
  ...
  refine ⟨MSIView.synced_step hs (msi_step_internal.cache x c' k e h), ?_, ?_, ?_, ?_⟩
  · intro k' hk'
...
/-- Trasferimento di `cohInv` da `x` a `y` quando `y` differisce da `x` solo all'indice `k`. -/
theorem coh_step_parent_aux1 {x y : MSIState n} {k : Fin n} (hc : cohInv x)
    (hs' : MSIView.synced y)
    (hyc : ∀ k', k' ≠ k → y.caches k' = x.caches k')
    (hyp : ∀ k', k' ≠ k → y.parent.shared_state k' = x.parent.shared_state k')
    (hkM : y.parent.shared_state k = Bstate.M → mtok y k = 1 ∧ stok y k = 0)
    (hkS : y.parent.shared_state k = Bstate.S → stok y k = 1 ∧ mtok y k = 0)
    (hkI : y.parent.shared_state k = Bstate.I → mtok y k = 0 ∧ stok y k = 0)
    (hexcl : y.parent.shared_state k = Bstate.M → ∀ k', k' ≠ k → x.parent.shared_state k' = Bstate.I)
    (hexcl' : ∀ k', k' ≠ k → x.parent.shared_state k' = Bstate.M → y.parent.shared_state k = Bstate.I) :
    cohInv y := by
...
theorem coh_step_parent_aux2 {x : MSIState n} {k : Fin n} {P : ParentState n} {a : PCEvent}
    (hc : cohInv x)
    (hs' : MSIView.synced { x with
    ...
    (ha : List.countP MSIView.isGrantM [a] = 0) (hb : List.countP MSIView.isGrantS [a] = 0) :
    cohInv { x with
      caches := update_Fin k { x.caches k with queue_cp := P.queue_cip k, queue_pc := P.queue_pci k } x.caches,
      parent := P } := by
  have hs := hc.synced
  ...
  refine coh_step_parent_aux1 hc hs' (fun k' hk' => update_Fin_gso2 _ _ _ _ hk')
    (fun k' hk' => (hloc k' hk').2.2) ?_ ?_ ?_ ?_ ?_
  · intro hr
    have hr' : x.parent.shared_state k = Bstate.M := by rw [← hrow]; exact hr
    unfold mtok stok
    simp only [update_Fin_gss]
    rw [hm, hst]
    exact hc.rowM k hr'
...
theorem coh_step_parent {x : MSIState n} {k : Fin n} {e : ParentUpdQueueInternalEvent n}
    {p' : ParentState n} (hc : cohInv x) (h : parent_msi_step x.parent (.upd_queue e k) p') :
    cohInv { x with caches := update_Fin k { x.caches k with queue_cp := p'.queue_cip k,
                                                              queue_pc := p'.queue_pci k } x.caches,
                    parent := p' } := by
  have hs' := MSIView.synced_step hc.synced (msi_step_internal.parent_upd_queue x p' e k h)
  have hloc := MSIView.parent_step_local h
  have hc0 := hc
  obtain ⟨hs, hM, hS, hI, hex⟩ := hc
  cases h with
  | downgrade_from_M_rq1 v i j hj =>
    -- rilascio da `M`: la riga di `k` era `M`, ora `I`, il token `rsIμ` sparisce
    refine coh_step_parent_aux1 hc0 hs' (fun k' hk' => update_Fin_gso2 _ _ _ _ hk')
      (fun k' hk' => (hloc k' hk').2.2) ?_ ?_ ?_ ?_ ?_
    · intro hr; exact (coh_step_parent_aux0 (by intro h; cases h) hr).elim
    · intro hr; exact (coh_step_parent_aux0 (by intro h; cases h) hr).elim
    · intro _
      rw [(hs k).1] at hj
      have e1 := countP_eraseIdx_pos (p := MSIView.isReleaseM) hj rfl
      have e2 := countP_eraseIdx_neg (p := MSIView.isReleaseS) hj rfl
      have hrow : x.parent.shared_state k = Bstate.M := by
  ...
  | downgrade_from_M_rq2 i j hj =>
    ...
    · intro _
      rw [(hs k).1] at hj
      have e1 := countP_eraseIdx_pos (p := MSIView.isReleaseS) hj rfl
      have e2 := countP_eraseIdx_neg (p := MSIView.isReleaseM) hj rfl
      have hrow : x.parent.shared_state k = Bstate.S := by
        cases hr : x.parent.shared_state k with
        | S => rfl
        | I => have := (hI k hr).2; unfold stok stokC at this; omega
        | M => have := (hM k hr).2; unfold stok stokC at this; omega
  ...
  | upgrade_to_M_data_avilable_rq1 i j hj hall =>
    ...
    · intro _
      rw [(hs k).1] at hj
      have e1 := countP_eraseIdx_neg (p := MSIView.isReleaseM) hj rfl
      have e2 := countP_eraseIdx_neg (p := MSIView.isReleaseS) hj rfl
      have e3 : List.countP MSIView.isGrantM [PCEvent.rsM x.parent.value] = 1 := rfl
      have e4 : List.countP MSIView.isGrantS [PCEvent.rsM x.parent.value] = 0 := rfl
    ...
    · intro k' _ hM'
      ...
      rw [hM'] at this
      cases this
  | upgrade_to_M_data_avilable_rq2 i j hj hIk hnoM =>
    ...
    · intro _
      ...
      have e1 := countP_eraseIdx_neg (p := MSIView.isReleaseM) hj rfl
      have e2 := countP_eraseIdx_neg (p := MSIView.isReleaseS) hj rfl
      have e3 : List.countP MSIView.isGrantM [PCEvent.rsS x.parent.value] = 0 := rfl
      have e4 : List.countP MSIView.isGrantS [PCEvent.rsS x.parent.value] = 1 := rfl
    ...
    · intro hr; exact (coh_step_parent_aux0 (by intro h; cases h) hr).elim
    · intro hr; exact (coh_step_parent_aux0 (by intro h; cases h) hr).elim
    · intro k' _ hM'; exact absurd hM' (hnoM k')
...
theorem coh_step {x y : MSIState n} {e : MSIInternalEvent n} (hc : cohInv x)
    (h : msi_step_internal x e y) : cohInv y := by
  cases h with
...
theorem coh_step_ext_aux1 {x y : MSIState n} (hc : cohInv x) (hs : MSIView.synced y)
...
theorem coh_step_ext {x y : MSIState n} {e : MSIExternalEvent n} (hc : cohInv x) (h : msi_step_external x e y) :
    cohInv y := by
  cases h with
  | cache ev c' k hc' =>
...
theorem lv_default : LV (default : MSIState n) 0 := by
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro k h
    change Bstate.I = Bstate.M at h
    cases h
...
/-- Ausiliario di `lv_step_cache`: una cache non in `I` ha il valore logico. -/
theorem lv_step_cache_aux1 {s1 : CacheState} {m : Value}
    (hcM : s1.state = Bstate.M → s1.value = m) (hcS : s1.state = Bstate.S → s1.value = m) :
...
theorem lv_step_cache {x : MSIState n} {m : Value} {k : Fin n} {e : CacheInternalEvent}
    {c' : CacheState} (_hc : cohInv x) (hl : LV x m) (h : cache_msi_step_internal (x.caches k) e c') :
  ...
  have hcv : (x.caches k).state ≠ Bstate.I → (x.caches k).value = m :=
    lv_step_cache_aux1 (hcM k) (hcS k)
  have hgv : ∀ v, (PCEvent.rsM v ∈ (x.caches k).queue_pc ∨ PCEvent.rsS v ∈ (x.caches k).queue_pc)
      → v = m := by
    intro v hv
    rcases hv with hv | hv
    · exact hgM _ v hv
    · exact hgS _ v hv
  have hval := inv_step_cache_aux3 h hcv hgv
...
/-- Ausiliario di `lv_step_parent`: i campi di `LV` sulle cache, dati quelli della cache `k` letti
dalle copie lato parent. -/
theorem lv_step_parent_aux1 {x : MSIState n} {m : Value} {k : Fin n} {p' : ParentState n}
    (hl : LV x m)
...
theorem lv_step_parent_aux2 {x : MSIState n} {k : Fin n} {p' : ParentState n}
    (hM : (p'.queue_pci k).countP MSIView.isGrantM = (x.caches k).queue_pc.countP MSIView.isGrantM)
    (hR : (p'.queue_cip k).countP MSIView.isReleaseM
            = (x.caches k).queue_cp.countP MSIView.isReleaseM)
    (h0 : ∀ k', mtok { x with
                caches := update_Fin k { x.caches k with queue_cp := p'.queue_cip k,
                                                          queue_pc := p'.queue_pci k } x.caches,
                parent := p' } k' = 0) :
...
theorem lv_step_parent {x : MSIState n} {m : Value} {k : Fin n} {e : ParentUpdQueueInternalEvent n}
    {p' : ParentState n} (hc : cohInv x) (hl : LV x m) (h : parent_msi_step x.parent (.upd_queue e k) p') :
    LV { x with caches := update_Fin k { x.caches k with queue_cp := p'.queue_cip k,
                                                          queue_pc := p'.queue_pci k } x.caches,
                parent := p' } m := by
  ...
  case downgrade_from_M_rq1 v j hj =>
    have hv : v = m := by
      ...
      rw [← hcp]; exact List.mem_of_getElem? hj
    refine lv_step_parent_aux1 hl ?_ ?_ ?_ ?_
    · intro v' hmem
      apply hl.grantM k v'
      rw [← hpc]; exact hmem
    · intro v' hmem
  ...
  case downgrade_from_M_rq2 j hj =>
    ...
    · intro h0
      ...
      · exact congrArg (List.countP MSIView.isGrantM) hpc
      · simp only [update_Fin_gss]
  ...
  all_goals
    ...
    · intro h0
      ...
      · simp only [update_Fin_gss, List.countP_append, hpc]
        rfl
      · exact congrArg (List.countP MSIView.isReleaseM) hcp

theorem lv_step {x y : MSIState n} {m : Value} {e : MSIInternalEvent n} (hc : cohInv x) (hl : LV x m)
    (h : msi_step_internal x e y) : LV y m := by
  cases h with
  | cache c' k e' hc' => exact lv_step_cache hc hl hc'
  | parent_upd_queue p' e' k hp => exact lv_step_parent hc hl hp
  | parent_no_queue _ _ _ hp => cases hp

theorem lv_step_ext_aux1 {x y : MSIState n} {m : Value} (hl : LV x m)
  ...
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro k hM; rw [hv]; exact h1 k ((hs k).symm.trans hM)
...
theorem lv_step_ext_aux2 {x : MSIState n} {m : Value} {k : Fin n} {c' : CacheState}
...
theorem lv_step_ext {x y : MSIState n} {m : Value} {e : MSIExternalEvent n} (hc : cohInv x) (hl : LV x m)
    (h : msi_step_external x e y) : ∃ m', LV y m' := by
  cases h with
  | cache ev c' k hc' =>
    cases hc' with
    ...
    | st_rq_M_state v rst hrq hM =>
      ...
      have hpos : mtok x k ≠ 0 := by
        unfold mtok mtokC; rw [hM, if_pos rfl]; omega
      have hrow : x.parent.shared_state k = Bstate.M := by
        have hr : x.parent.shared_state k = Bstate.M ∨ x.parent.shared_state k = Bstate.I
            ∨ x.parent.shared_state k = Bstate.S := by
      ...
      have hcnt : (x.caches k).queue_pc.countP MSIView.isGrantM = 0 ∧
          (x.caches k).queue_cp.countP MSIView.isReleaseM = 0 := by
...
theorem coh_lv_trans {x y : MSIState n} {m : Value} (hc : cohInv x) (hl : LV x m)
    (h : trans_refl (msi_rule n) x y) : cohInv y ∧ LV y m := by
  revert hc hl
  induction h with
...
theorem reach_inv {x : MSIState n} (h : Reach x) : cohInv x ∧ ∃ m, LV x m := by
...
theorem canon_trans {x y : MSIState n} {m : Value} (h : trans_refl (msi_rule n) x y) :
...
theorem predrain_aux1 {x y : MSIState n} {k : Fin n} (hc : cohInv x)
...
theorem predrain_aux2 {x : MSIState n} {k : Fin n} (hc : cohInv x)
    (hk : x.parent.shared_state k = Bstate.M) (hM : (x.caches k).state = Bstate.M)
    (hg : (x.caches k).queue_pc.countP MSIView.isGrantM = 0)
    (hr : (x.caches k).queue_cp.countP MSIView.isReleaseM = 0) :
    ∃ y, trans_refl (msi_rule n) x y ∧ ∀ k, mtok y k = 0 := by
  have hs := hc.synced
  ...
  obtain ⟨x1, step1, h1s, h1state, h1pc, h1cp, h1oth⟩ :
      ∃ x1, msi_rule n x x1
        ...
        ∧ (∀ k', k' ≠ k → x1.caches k' = x.caches k') := by
    have hstep := msi_step_internal.cache x _ k _
      (cache_msi_step_internal.rq_data_not_available (x.caches k) hM)
    refine ⟨_, msi_rule_of_step hstep, MSIView.synced_step hs hstep, ?_, ?_, ?_, ?_⟩
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]
  ...
  obtain ⟨x2, step2, h2state, h2pc, h2cp, h2oth⟩ :
    ...
    have hstep := msi_step_internal.parent_upd_queue x1 _ _ k
      (parent_msi_step.downgrade_from_M_rq1 x1.parent (x.caches k).value k _ hg6)
    refine ⟨_, msi_rule_of_step hstep, ?_, ?_, ?_, ?_⟩
...
theorem predrain_aux3 {x : MSIState n} {k : Fin n} (hc : cohInv x)
  ...
  have hI : (x.caches k).state = Bstate.I := by
    cases hst : (x.caches k).state with
    | M => exact absurd hst hnM
    | S => exact absurd hst hnS
    | I => rfl
  obtain ⟨j, hj⟩ := List.mem_iff_getElem?.mp hv
  have hg1 : (x.caches k).queue_pc.countP MSIView.isGrantM = 1 := by
    have h1' := h1
    unfold mtok mtokC at h1'
    rw [if_neg hnM, hr] at h1'
    omega
  have hgE : ((x.caches k).queue_pc.eraseIdx j).countP MSIView.isGrantM = 0 := by
    have := countP_eraseIdx_pos (p := MSIView.isGrantM) hj rfl
    omega
  -- Passo 1: la cache `k` prende il grant.
  obtain ⟨x1, step1, h1s, h1state, h1pc, h1cp, h1val, h1oth⟩ :
      ∃ x1, msi_rule n x x1
        ∧ MSIView.synced x1
        ∧ (x1.caches k).state = Bstate.M
        ∧ (x1.caches k).queue_pc = (x.caches k).queue_pc.eraseIdx j
        ∧ (x1.caches k).queue_cp = (x.caches k).queue_cp
        ∧ (x1.caches k).value = v
        ∧ (∀ k', k' ≠ k → x1.caches k' = x.caches k') := by
    have hstep := msi_step_internal.cache x _ k _
      (cache_msi_step_internal.upgrade_from_I_rs (x.caches k) v j hj hI)
    refine ⟨_, msi_rule_of_step hstep, MSIView.synced_step hs hstep, ?_, ?_, ?_, ?_, ?_⟩
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]
    · intro k' hk; simp only [update_Fin_gso2 _ _ _ _ hk]
  -- Passo 2: la cache `k` rilascia spontaneamente.
  obtain ⟨x2, step2, h2s, h2state, h2pc, h2cp, h2oth⟩ :
      ∃ x2, msi_rule n x1 x2
        ∧ MSIView.synced x2
        ∧ (x2.caches k).state = Bstate.I
        ∧ (x2.caches k).queue_pc = (x.caches k).queue_pc.eraseIdx j
        ∧ (x2.caches k).queue_cp = (x.caches k).queue_cp ++ [CPEvent.rsIμ v]
        ∧ (∀ k', k' ≠ k → x2.caches k' = x.caches k') := by
    have hstep := msi_step_internal.cache x1 _ k _
      (cache_msi_step_internal.rq_data_not_available (x1.caches k) h1state)
    refine ⟨_, msi_rule_of_step hstep, MSIView.synced_step h1s hstep, ?_, ?_, ?_, ?_⟩
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]; exact h1pc
  ...
  obtain ⟨x3, step3, h3state, h3pc, h3cp, h3oth⟩ :
      ∃ x3, msi_rule n x2 x3
        ∧ (x3.caches k).state = Bstate.I
        ∧ (x3.caches k).queue_pc = (x.caches k).queue_pc.eraseIdx j
    ...
    have hstep := msi_step_internal.parent_upd_queue x2 _ _ k
      (parent_msi_step.downgrade_from_M_rq1 x2.parent v k _ hg6)
    refine ⟨_, msi_rule_of_step hstep, ?_, ?_, ?_, ?_⟩
  ...
  refine predrain_aux1 hc hk ?_ h3oth
  unfold mtokC
  rw [h3state, h3pc, h3cp, hgE, hr]
  simp

/-- Caso C: un rilascio `rsIμ v` in volo da `k`; il parent lo prende. -/
theorem predrain_aux4 {x : MSIState n} {k : Fin n} (hc : cohInv x)
    (hk : x.parent.shared_state k = Bstate.M) (hnM : (x.caches k).state ≠ Bstate.M)
    (hg : (x.caches k).queue_pc.countP MSIView.isGrantM = 0)
  ...
  obtain ⟨j, hj⟩ := List.mem_iff_getElem?.mp hv
  have hr1 : (x.caches k).queue_cp.countP MSIView.isReleaseM = 1 := by
    have h1' := h1
    unfold mtok mtokC at h1'
    rw [if_neg hnM, hg] at h1'
    omega
  have hrE : ((x.caches k).queue_cp.eraseIdx j).countP MSIView.isReleaseM = 0 := by
    have := countP_eraseIdx_pos (p := MSIView.isReleaseM) hj rfl
    omega
  ...
  obtain ⟨x1, step1, h1state, h1pc, h1cp, h1oth⟩ :
      ∃ x1, msi_rule n x x1
        ...
        ∧ (∀ k', k' ≠ k → x1.caches k' = x.caches k') := by
    have hstep := msi_step_internal.parent_upd_queue x _ _ k
      (parent_msi_step.downgrade_from_M_rq1 x.parent v k j hj')
    refine ⟨_, msi_rule_of_step hstep, ?_, ?_, ?_, ?_⟩
    · simp only [update_Fin_gss]
    · simp only [update_Fin_gss]; rw [(hs k).2]
    · simp only [update_Fin_gss]; rw [(hs k).1]
  ...
  refine predrain_aux1 hc hk ?_ h1oth
  unfold mtokC
  rw [h1state, if_neg hnM, h1pc, h1cp, hg, hrE]


/-- Si completa il ciclo dell'eventuale token `M` in giro: alla fine nessun token `M`. -/
theorem predrain {x : MSIState n} {m : Value} (hc : cohInv x) (_hl : LV x m) :
    ∃ y, trans_refl (msi_rule n) x y ∧ ∀ k, mtok y k = 0 := by
  by_cases hex : ∃ k, x.parent.shared_state k = Bstate.M
  · obtain ⟨k, hk⟩ := hex
    have ⟨h1, _⟩ := hc.rowM k hk
    rcases mtok_one_cases h1 with ⟨hM, hg, hr⟩ | ⟨hnM, ⟨v, hv⟩, hr⟩ | ⟨hnM, hg, ⟨v, hv⟩⟩
    · exact predrain_aux2 hc hk hM hg hr
    · exact predrain_aux3 hc hk hnM hv hr
    · exact predrain_aux4 hc hk hnM hg hv
  · simp only [not_exists] at hex
    refine ⟨x, trans_refl.refl, fun k => ?_⟩
    cases hrow : x.parent.shared_state k with
    | M => exact absurd hrow (hex k)
...
theorem flushInv_of_lv {x : MSIState n} {m : Value} (hc : cohInv x) (hl : LV x m)
    (h0 : ∀ k, mtok x k = 0) : flushInv x ⟨m, fun _ => default⟩ := by
  refine ⟨hc.synced, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · -- rowM: nessuna riga può essere `M` senza token `M`
    intro k hrow
  ...
  · -- cacheVal
    intro k hne
    cases hst : (x.caches k).state with
    | M => exact absurd hst (mtok_zero_facts (h0 k)).1
    | I => exact absurd hst hne
    | S => exact hl.cacheS k hst
  · -- grantVal
    intro k v hmem
    rcases hmem with h | h
    · exact absurd h ((mtok_zero_facts (h0 k)).2.1 v)
...
theorem fix_step {x : MSIState n} {s : SeqState n} {k : Fin n} (hf : flush0 x s)
    (hk : (x.caches k).value ≠ s.memory) :
    ∃ y, trans_refl (msi_rule n) x y ∧ flush0 y s ∧ (y.caches k).value = s.memory
      ∧ (y.caches k).extqueue = (x.caches k).extqueue
      ∧ ∀ k', k' ≠ k → y.caches k' = x.caches k' := by
  obtain ⟨hc, hp, hv⟩ := hf
  obtain ⟨hkI, hcp, hpc⟩ := hc k
  refine ⟨_, trans_refl.step
    (msi_rule_of_step (msi_step_internal.cache _ _ k _
      (cache_msi_step_internal.upgrade_from_I_rq _ hkI)))
    (trans_refl.step
      (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
        (parent_msi_step.upgrade_to_M_data_avilable_rq1 _ k 0 ?h1 ?h2)))
      (trans_refl.step
        (msi_rule_of_step (msi_step_internal.cache _ _ k _
          (cache_msi_step_internal.upgrade_from_I_rs _ x.parent.value 0 ?h3 ?h4)))
        (trans_refl.step
          (msi_rule_of_step (msi_step_internal.cache _ _ k _
            (cache_msi_step_internal.rq_data_not_available _ ?h5)))
          (trans_refl.step
            (msi_rule_of_step (msi_step_internal.parent_upd_queue _ _ _ k
              (parent_msi_step.downgrade_from_M_rq1 _ x.parent.value k 0 ?h6)))
...
theorem fix_all {x : MSIState n} {s : SeqState n} (hf : flush0 x s) :
    ∃ y, trans_refl (msi_rule n) x y ∧ flush0 y s ∧ (∀ k, (y.caches k).value = s.memory)
      ∧ ∀ k, (y.caches k).extqueue = (x.caches k).extqueue := by
  suffices H : ∀ N, ∀ x : MSIState n, flush0 x s → μ4 x s.memory ≤ N →
      ∃ y, trans_refl (msi_rule n) x y ∧ flush0 y s ∧ (∀ k, (y.caches k).value = s.memory)
...
theorem canon_of_flush {x : MSIState n} {s : SeqState n} (hf : flush0 x s)
  ...
  have hparent : x.parent = ⟨s.memory, fun _ => Bstate.I, fun _ => [], fun _ => []⟩ := by
    ...
    | mk pv rows cip pci =>
      ...
      subst hpv e1 e2 e3
      rfl
  have h : (⟨x.caches, x.parent⟩ : MSIState n) = canon x s.memory := by
    unfold canon
    rw [hparent]
    congr 1
  exact h
...
theorem reach_canon {x : MSIState n} {m : Value} (hc : cohInv x) (hl : LV x m) :
    trans_refl (msi_rule n) x (canon x m) := by
  ...
  have hpath : trans_refl (msi_rule n) x y5 :=
...
theorem coh_of_flush0 {x : MSIState n} {s : SeqState n} (hf : flush0 x s) : cohInv x := by
  obtain ⟨hc, hp, hv⟩ := hf
  refine ⟨?_, ?_, ?_, ?_, ?_⟩
  · intro k
...
theorem lv_of_flush0 {x : MSIState n} {s : SeqState n} (hf : flush0 x s) : LV x s.memory := by
  obtain ⟨hc, hp, hv⟩ := hf
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro k h
    rw [(hc k).1] at h
...
theorem flush_canon {x : MSIState n} {m : Value} {s : SeqState n} (hm : s.memory = m)
...
theorem lv_step_ext_val {x y : MSIState n} {m : Value} {e : Event} {k : Fin n} (hc : cohInv x)
    (hl : LV x m) (h : msi_step_external x (.cache e k) y) :
...
theorem acquire_M {x : MSIState n} {s : SeqState n} (k : Fin n) (hf : flush0 x s) :
    ∃ y, trans_refl (msi_rule n) x y ∧ (y.caches k).state = Bstate.M
...
/-- `relation_flush` di ARS per la `flush` completa: `msi_relation_flush0` più l'invarianza delle
...
theorem msi_relation_flush (i i' : MSIState n) (s : SeqState n) :
  ...
  intro hf htr
  obtain ⟨hf0, hext⟩ := hf
  obtain ⟨i'', h1, h2⟩ := msi_relation_flush0 i i' s hf0 htr
...
theorem msi_relation_init : relation_init flush (default : MSIState n) (seq_init n) := by
  ...
  exact ⟨msi_relation_init0 default
...
theorem msi_commutes_upto_aux1 {x y : MSIState n} {m : Value}
...
theorem msi_commutes_upto_aux2 {c d : MSIState n} {m : Value} (hcc : cohInv c) (hlc : LV c m)
  ...
  rw [msi_commutes_upto_aux1 hext]
...
theorem msi_commutes_upto_aux3 {b : MSIState n} {m : Value} (k : Fin n) (hcb : cohInv b)
    (hlb : LV b m) :
    ∃ f, trans_refl (msi_rule n) b f ∧ (f.caches k).state = Bstate.M ∧ (f.caches k).value = m
  ...
  have hpath : trans_refl (msi_rule n) b f :=
...
theorem msi_commutes_upto :
  ...
  | ld_rq =>
    ...
    obtain ⟨j, hcj, hdj⟩ := msi_commutes_upto_aux2 (coh_step_ext hca hac')
  ...
  | ld_rq_data_available1 rst hrq hS =>
    ...
    obtain ⟨f, hbf, hfM, hfv, hfext⟩ := msi_commutes_upto_aux3 k hcb hlb
  ...
  | ld_rq_data_available rst hrq hM =>
    ...
    have hva : (a.caches k).value = m := hla.cacheM k hM
    obtain ⟨f, hbf, hfM, hfv, hfext⟩ := msi_commutes_upto_aux3 k hcb hlb
    obtain ⟨hcf, hlf⟩ := coh_lv_trans hcb hlb hbf
    have hextaf : ∀ k', (f.caches k').extqueue = (a.caches k').extqueue :=
      fun k' => (hfext k').trans (hextab k')
  ...
  | st_rq_M_state v rst hrq hM =>
    ...
    obtain ⟨f, hbf, hfM, _, hfext⟩ := msi_commutes_upto_aux3 k hcb hlb
    obtain ⟨hcf, hlf⟩ := coh_lv_trans hcb hlb hbf
    have hextaf : ∀ k', (f.caches k').extqueue = (a.caches k').extqueue :=
      fun k' => (hfext k').trans (hextab k')
    ...
    obtain ⟨j, hcj, hdj⟩ := msi_commutes_upto_aux2 (coh_step_ext hca hac') hlc
...
(commutazione a meno di passi interni). Le sei ipotesi: (1) `msi_relation_flush`, (2)
`msi_relation_flush_method_int`, (3) `msi_relation_method_int`, (4) `msi_confluent`, (5)
`msi_commutes_upto`, (6) `msi_relation_init`. Le conversioni tra `star_extend`/`star` di `star.lean`
...
theorem trace_inclusion (l : List (MSIExternalEvent n)) :
  ...
  -- a `star_extend` di ARS (tratti `trans_refl (msi_rule n)`)
